 ...
 */
public class SecureFrameIOSender{
	...
	private CryptProvider crpyt;
	private AES128Provider aes;
	private byte[]key;
	/**
	...
	public void run() {
		...
		try {
			...
			final FrameSecurity framesecure= new FrameSecurity(crpyt,10,1);
			final SecureFrameIO secureIO = new SecureFrameIO(fio,framesecure);

			// start transmitter
			startTransmitter(secureIO);
...
//		System.out.println("Done(Init)");
	 ...
	 * instance.
	 */
	public void startTransmitter(final SecureFrameIO secureIO)throws Exception  {
		...
		new Thread() {
			...
			public void run() {
				...
				for (;;) {
					try {
						// transmit a frame
						framesecure.encrypt(testFrame,1);
						secureIO.transmit(testFrame);
